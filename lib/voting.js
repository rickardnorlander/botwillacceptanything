...
module.exports = function(config, gh, Twitter) {
  ...
  function timeSinceStart (pr) {
    ...
    getVoteStartedComment(pr, function () {
    });
    ...
    getVoteStartedComment(pr, function(err, comment) {
      ...
      if(comment) {
        ...
        if(pr.head.sha !== originalHead) {
          console.log('Posting a "modified PR" warning, and restarting voting #' + pr.number);
          return;
        }
      }
  ...
  function processPR(pr) {
    ...
    if (timeSinceStart(pr) / MINUTE < PERIOD) return;
  ...
  function tallyVotes(pr) {
    var tally = pr.comments.reduce(function (result, comment) {
      ...
      var isRestart = pr.comments[i].body.indexOf(modifiedWarning) === 0;
      if (isMe && isRestart) {
        return {};
      }
