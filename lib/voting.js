...
var MIN_VOTES = 2; // minimum number of votes for a decision to be made
...
var voteStartedComment = '#### :ballot_box_with_check: Voting procedure reminder:\n' +
  ...
  '*NOTE: the vote will be restarted if any new commits are added after:* ';

var modifiedWarning = '#### :warning: This PR has been modified so voting will now restart.\n\n*NOTE: the vote will be restarted again if any new commits are added after:* '
...
module.exports = function(config, gh, Twitter) {
  ...
  function handlePR(pr) {
    ...
    assertNotModified(pr, function() {
      ...
      var age = Date.now() - timeSinceStart(pr);
  ...
  function timeSinceStart (pr) {
    var t0 = new Date(pr.created_at).getTime();
    getVoteStartedComment(pr, function (err, comment) {
	if (comment != null) {
	    t0 = new Date (comment.created_at).getTime();
	}
    });
    return Date.now() - t0;
  }
    ...
    getVoteStartedComment(pr, function(err, comment) {
      ...
      if(comment) {
        ...
        if(pr.head.sha !== originalHead) {
          console.log('Posting a "modified PR" warning, and restarting voting #' + pr.number+""+pr.head.sha+" "+originalHead);
  ...
  function tallyVotes(pr) {
    var tally = pr.comments.reduce(function (result, comment) {
      var user = comment.user.login,
          body = comment.body;
      var isMe = user === config.user;
      var isRestart = comment.body.indexOf(modifiedWarning) === 0;
      if (isMe && isRestart) {
        return {};
      }
